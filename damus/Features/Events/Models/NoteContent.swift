...
struct LongformContent {
    ...
    private static func findExcludedRanges(in markdown: String) -> [Range<String.Index>] {
        ...
        let fencedPattern = #"(?:^|\n)(`{3,}|~{3,}).*?(?:\n\1|\z)"#
        if let fencedRegex = try? NSRegularExpression(pattern: fencedPattern, options: [.dotMatchesLineSeparators]) {
            let matches = fencedRegex.matches(in: markdown, options: [], range: NSRange(markdown.startIndex..., in: markdown))
        ...
        let indentedPattern = #"(?:^|\n[ \t]*\n)((?:(?:    |\t).+\n?)+)"#
        if let indentedRegex = try? NSRegularExpression(pattern: indentedPattern, options: []) {
            let matches = indentedRegex.matches(in: markdown, options: [], range: NSRange(markdown.startIndex..., in: markdown))
            for match in matches {
                // Use the captured group (the actual code content)
                if match.numberOfRanges > 1, let range = Range(match.range(at: 1), in: markdown) {
                    ranges.append(range)
                }
            }
        }
        ...
        if let inlineRegex = try? NSRegularExpression(pattern: inlinePattern, options: []) {
            let matches = inlineRegex.matches(in: markdown, options: [], range: NSRange(markdown.startIndex..., in: markdown))
            for match in matches {
                if let range = Range(match.range, in: markdown) {
                    ranges.append(range)
                }
